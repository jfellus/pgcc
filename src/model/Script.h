...
public:
	...
	void compute_threads() {
		...
		for(uint c=0; c<modules.size(); c++) {
			...
			std::list<Module*> comp;
			for(uint j=0; j<modules.size(); j++) if(modules[j]->thread==0 && modules[j]->is_processable()) {comp.push_back(modules[j]); break;}
			if(!comp.empty()) thread = threads.add(new Thread(this, threads.size()));
			while(!comp.empty()) {
				Module* m = comp.front(); comp.pop_front();
				thread->add(m); n++;
				if(n>=modules.size()) {
					if(thread && thread->nbProcessables==0) {threads.remove(thread); delete thread; thread = 0;}
					return;
				...
				for(uint i=0; i<m->ins.size(); i++) {
					...
					if(m->ins[i]->src->thread==0) comp.push_back(m->ins[i]->src);
				}
				for(uint i=0; i<m->outs.size(); i++) {
					if(!m->outs[i]->is_same_thread()) continue;
					if(m->outs[i]->dst->thread==0) comp.push_back(m->outs[i]->dst);
				}
			}
		}
	}
